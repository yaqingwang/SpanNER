...
from src.common.ranker_base import BertEncoder, get_model_obj
from src.common.optimizer import get_bert_optimizer
from src.biencoder.allennlp_span_utils import batched_span_select, batched_index_select
from src.biencoder.utils import batch_reshape_mask_left
import pdb
import math
...
class MentionScoresHead(nn.Module):
    ...
    def forward(self, bert_output, mask_ctxt, start_label_mask, end_label_mask):
        '''
        ...
        if self.scoring_method[:2] == "qa":
            ...
            if start_label_mask is not None:
                start_logprobs[~(start_label_mask.bool())]  = -float("Inf")
            end_logprobs[~mask_ctxt] = -float("Inf")
            if end_label_mask is not None:
                end_logprobs[~(end_label_mask.bool())] = -float("Inf")
            ...
            # (bs, starts, ends)
            mention_cum_scores = torch.zeros(mention_scores.size(), dtype=mention_scores.dtype).to(
                mention_scores.device)
            # add ends
            mention_logprobs_end_cumsum = torch.zeros(mask_ctxt.size(0), dtype=mention_scores.dtype).to(
                mention_scores.device)
            ...
            # subtract starts
            mention_logprobs_start_cumsum = torch.zeros(mask_ctxt.size(0), dtype=mention_scores.dtype).to(
                mention_scores.device)
            ...
            mention_bounds = torch.stack([
                torch.arange(mention_scores.size(1)).unsqueeze(-1).expand(mention_scores.size(1),
                                                                          mention_scores.size(2)),  # start idxs
                torch.arange(mention_scores.size(1)).unsqueeze(0).expand(mention_scores.size(1),
                                                                         mention_scores.size(2)),  # end idxs
...
class BiEncoderModule(torch.nn.Module):
    def __init__(self, params):
        ...
        #params['bert_model'] = "bert-base-cased"
        model_type = params['bert_model'].split('-')[0]
        if model_type in MODEL_CLASSES:
            Config, pre_model, tokenizer = MODEL_CLASSES[model_type]
        else:
            print("No implement")
        self.tokenizer = tokenizer

        ctxt_bert = pre_model.from_pretrained(params["bert_model"], output_hidden_states=True)
        if params["load_cand_enc_only"]:
            bert_model = "bert-large-uncased"
            cand_encs = torch.load(params['cand_enc_path'])
            if params['use_attention']:
                self.cand_bert_output_dim = cand_encs[0][0].size(-1)
            else:
                self.cand_bert_output_dim = cand_encs[0].size(-1)
        else:
        ...
        if self.mention_aggregation_type is not None:
            ...
            self.linear_compression = nn.Linear(ctxt_bert_output_dim, self.cand_bert_output_dim)

        self.multi_headedAttn = MultiHeadedAttn(in_dim=self.cand_bert_output_dim)
    ...
    def get_raw_ctxt_encoding(
        ...
        raw_ctxt_encoding, _, _ = self.context_encoder.bert_model(
            input_ids=token_idx_ctxt,  attention_mask=mask_ctxt,
        )
    ...
    def get_ctxt_mention_scores(
        ...
        return self.classification_heads['mention_scores'](
            raw_ctxt_encoding, mask_ctxt, start_label_mask=start_label_mask, end_label_mask=end_label_mask
        )
    ...
    def prune_ctxt_mentions(
        ...
        if threshold is None:
            top_mention_pos_mask = top_mention_logits > 0
            start_pos = start_logits > 0
            end_pos = end_logits > 0

        else:

            top_mention_pos_mask = top_mention_logits > threshold
            start_pos = start_logits > threshold
            end_pos = end_logits > threshold

        seq_len = start_pos.size(-1)

        match_label_row = start_pos.unsqueeze(-1).expand(-1, -1, seq_len)
        match_label_col = end_pos.unsqueeze(-2).expand(-1, seq_len, -1)
        match_label = torch.triu(match_label_row & match_label_col)

        # match_label_position = torch.nonzero(match_label)
        ...
        chosen_mention_bounds, chosen_mention_mask = batch_reshape_mask_left(mention_bounds, mention_pos_mask,
                                                                             pad_idx=0)
        # (bsz, max_num_pred_mentions)ncode_e
        ...
        # pdb.set_trace()
        start_end_mask = torch.zeros(chosen_mention_mask.size(), dtype=torch.bool).to(chosen_mention_mask)

        for i in range(chosen_mention_bounds.size(0)):
            row_mention_bounds = chosen_mention_bounds[i]
            start_end_mask[i] = match_label[i][row_mention_bounds[:, 0], row_mention_bounds[:, 1]]

        chosen_mention_mask = chosen_mention_mask & start_end_mask
    ...
    def forward_ctxt(
        ...
        if self.mention_aggregation_type is None:
            ...
            if get_mention_scores:
                mention_logits, mention_bounds, start_logits, end_logits = self.get_ctxt_mention_scores(
                    token_idx_ctxt, segment_idx_ctxt, mask_ctxt, raw_ctxt_encoding,  start_label_mask=start_label_mask,
                     end_label_mask=end_label_mask
                )
                extra_rets['all_mention_logits'] = mention_logits
                extra_rets['all_mention_start_logits'] = start_logits
                extra_rets['all_mention_end_logits'] = end_logits
                ...
                if gold_mention_bounds is None:
                    (
                        ...
                        mention_logits, mention_bounds, num_cand_mentions, topK_threshold, start_logits, end_logits, start_label_mask=start_label_mask, end_label_mask=end_label_mask
                    )
...
class BiEncoderRanker(torch.nn.Module):
    def __init__(self, params, shared=None):
       ...
       #          params["bert_model"], do_lower_case=params["lowercase"])
       # else:
        self.tokenizer = tokenizer.from_pretrained(params["bert_model"])
        self.NULL_IDX = 0  ###fix
        #self.NULL_IDX = 1
        ...
        # self.multi_headedAttn = MultiHeadedAttn(in_dim=self.model.cand_bert_output_dim)
        ...
        # self.multi_headedAttn = self.multi_headedAttn.to(self.device)
    ...
    # if cand_enc_only:
    #     cand_state_dict = get_submodel_from_state_dict(state_dict, 'cand_aencoder')
    #     if self.model.cand_encoder is not None:
    ...
    def encode_context(
            ...
            get_mention_scores=True, start_label_mask=None, end_label_mask=None
    ):
        """
        ...
        if context_outs['mention_dims'].size(0) <= 1:
            for key in context_outs:
                if 'all' in key or key == 'mention_dims':
                    continue
                context_outs[key] = context_outs[key].view(
                    [context_outs['mention_dims'][0, 0], -1] + list(context_outs[key].size()[1:]))
            return context_outs
    ...
    def score_candidate(
        ...
        mention_logits = None
        mention_bounds = None
        start_mention_logits = None
        end_mention_logits = None
        if get_mention_scores:
        ...
        if hard_negs:
            ...
            # pdb.set_trace()
            if self.use_attention and len(embedding_cands.size()) == 3:
                all_cand_attention_mask = all_cand_attention_mask[hard_negs_mask]
                if len(embedding_ctxt.size()) == 1:
                    embedding_ctxt = embedding_ctxt.unsqueeze(0)
                if embedding_ctxt.size(0) == 0:
                    scores = embedding_ctxt.mm(embedding_cands.mean(1).t())
                    return scores, mention_logits, mention_bounds, start_mention_logits, end_mention_logits

                try:
                    embedding_cands = self.model.multi_headedAttn(embedding_ctxt.unsqueeze(1), embedding_cands,
                                                            embedding_cands,
                                                            all_cand_attention_mask)  # num_mention_in_batch x  T x embed_size
                except:
                    import pdb
                    pdb.set_trace()
            ...
            return all_scores, mention_logits, mention_bounds, start_mention_logits, end_mention_logits
        ...
        try:
            ...
            if self.use_attention:
                if len(embedding_ctxt.size()) == 1:
                    embedding_ctxt = embedding_ctxt.unsqueeze(0)
                if embedding_ctxt.size(0) == 0:
                    return embedding_ctxt.mm(embedding_cands.mean(1).t())
                for i in range(embedding_cands.size(0)):
                    embedding_ctxt = embedding_ctxt.to(self.device)

                    new_embedding_cands = embedding_cands[i, :, :].unsqueeze(0).repeat(embedding_ctxt.size(0), 1, 1).to(
                        self.device)
                    new_cand_attention_mask = cand_attention_mask[i, :].unsqueeze(0).repeat(embedding_ctxt.size(0),
                                                                                            1).to(self.device)

                    new_embedding_cands = self.model.multi_headedAttn(embedding_ctxt.unsqueeze(1), new_embedding_cands,
                                                                new_embedding_cands,
                                                                new_cand_attention_mask)  # num_mention_in_batch x  T x embed_size
                    new_embedding_ctxt = embedding_ctxt.unsqueeze(1)  # num_mention_in_batch x 1 x embed_size
                    new_embedding_cands = new_embedding_cands.unsqueeze(2)  # num_mention_in_batch x embed_size x 1
                    score = torch.bmm(new_embedding_ctxt, new_embedding_cands)  # num_mention_in_batch x 1 x 1
                    score = torch.squeeze(score).unsqueeze(-1)
                    scores.append(score)
    ...
    def forward(
        ...
        '''
        scores, out_mention_logits, out_mention_bounds, start_logits, end_logits = self.score_candidate(
            ...
            all_cand_attention_mask=all_cand_attention_mask,start_label_mask=start_label_mask,
                end_label_mask=end_label_mask
        )
        ...
        if start_mention_logits is None:
            start_mention_logits = start_logits
        if end_mention_logits is None:
            end_mention_logits = end_logits
        ...
        start_loss = 0
        end_loss = 0
        total_num_neg = (start_mention_logits != -float("inf")).sum()
        if mention_logits is not None and mention_bounds is not None:
            ...
            span_loss = self.get_span_loss(
                ...
                total_num_neg=total_num_neg
            )
            start_loss, end_loss = self.get_start_end_loss(
    ...
    def get_span_loss(
            self, gold_mention_bounds, gold_mention_bounds_mask, mention_logits, mention_bounds, partial_loss=False,
            total_num_neg=None
    ):
        """
        ...
        gold_mention_binary = torch.zeros(mention_logits.size(), dtype=mention_logits.dtype).to(
            gold_mention_bounds.device)
        ...
        if partial_loss:
            # pdb.set_trace()
            if total_num_neg is None:
                total_num = 20 * gold_mention_bounds.size(0)
            else:
                total_num = total_num_neg

            neg_num = (total_num - masked_gold_mention_binary.sum())
            neg_idx = torch.nonzero(1 - masked_gold_mention_binary, as_tuple=False).squeeze()
            pos_idx = torch.nonzero(masked_gold_mention_binary, as_tuple=False)
            if len(pos_idx.size()) == 2:
                pos_idx = pos_idx.squeeze()

            choice = torch.randperm(neg_idx.size(0))[:int(neg_num)]
            # choice = torch.randperm(neg_idx.size(0))

            select_neg_idx = neg_idx[choice]

            if masked_gold_mention_binary.sum() == 0:
                entire_idx = select_neg_idx
            else:
                try:
                    if len(pos_idx.size()) != len(select_neg_idx.size()):
                        pos_idx = pos_idx.unsqueeze(0)
                    entire_idx = torch.cat((pos_idx, select_neg_idx))
                except:
                    pdb.set_trace()
            float_match_label_mask = torch.zeros(masked_mention_logits.size(), dtype=masked_mention_logits.dtype).to(
                masked_mention_logits.device)
            float_match_label_mask[entire_idx] = 1
            mask = float_match_label_mask.bool()
            masked_mention_logits = masked_mention_logits[mask]
            masked_gold_mention_binary = masked_gold_mention_binary[mask]
    ...
    def get_start_end_loss(
            self, start_mention_logits=None,
            start_label=None,
            start_label_mask=None,
            end_mention_logits=None,
            end_label=None,
        ...
        start_mask = (start_mention_logits != -float("inf"))
        if start_label_mask is not None:
            start_mask = start_label_mask.bool() & start_mask
        masked_start_logits = start_mention_logits[start_mask]
        masked_start_label = start_label[start_mask]

        end_mask = (end_mention_logits != -float("inf"))
        if end_label_mask is not None:
            end_mask = end_label_mask.bool() & end_mask
        masked_end_logits = end_mention_logits[end_mask]
        masked_end_label = end_label[end_mask]

        start_loss = loss_fct(masked_start_logits, masked_start_label.float())
        end_loss = loss_fct(masked_end_logits, masked_end_label.float())
...
class MultiHeadedAttn(torch.nn.Module):
    def __init__(self, heads=4, in_dim=768, d_model=300, dropout=0.2):
        ...
        self.fwQ = nn.Linear(in_dim, d_model)  # for the query
        self.fwK = nn.Linear(in_dim, d_model)  # for the key
        self.fwV = nn.Linear(in_dim, d_model)  # for the answer
        # W0
        self.fwMH = nn.Linear(d_model, in_dim)  # output linear layer
    ...
    def forward(self, query, key, value, mask=None):
        ...
        ##
        if x.size(0) == 1:
            out = self.fwMH(x).squeeze()
            out = out.unsqueeze(0)

        else:
            out = self.fwMH(x).squeeze()  # [batch_size, sentence_length, d_model]
        return out