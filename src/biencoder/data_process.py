...
from src.common.params import ENT_START_TAG, ENT_END_TAG, ENT_TITLE_TAG
import pdb
import pdb
import pdb
...
def get_context_representation_multiple_mentions_idxs(
    ...
    # if position ids not sorted, sorted by this part
    if len(mention_idxs) == 0:
        input_ids_window = start_token + input_ids[:(max_seq_length-2)]  + end_token
        ...
        context_left_len = len(input_ids[:mention_idxs[0][0]])
        if context_left_len + 2 > max_seq_length:
            input_ids_window = start_token + input_ids[:(max_seq_length-2)]  + end_token
            mention_idxs = []
        else:
            all_mention_spans_range = [mention_idxs[0][0], mention_idxs[-1][1]]
            while all_mention_spans_range[1] - all_mention_spans_range[0] + 2 + context_left_len > max_seq_length:
                if len(mention_idxs) == 1:
                    # don't cut further
                    assert mention_idxs[0][1] +  2 + context_left_len  > max_seq_length
                    assert mention_idxs[0][0] + 2  < max_seq_length
                    # truncate mention
                    mention_idxs[0][1] = max_seq_length  - 2
                else:
                    # cut last mention
    ...
    padding = [pad_id] * (max_seq_length - len(input_ids_window))
...
def process_mention_data(
    ...
    for idx, sample in enumerate(iter_):
        ...
        start_label_mask = sample.get("start_label_mask")
        end_label_mask = sample.get("end_label_mask")
        if len(start_label_mask) != 0:
            start_label_mask = [0] + start_label_mask
            end_label_mask = [0] + end_label_mask
            padding = [0] * (max_context_length - len(start_label_mask))
            start_label_mask = start_label_mask[:max_context_length] + padding
            end_label_mask = end_label_mask[:max_context_length] + padding
            start_label_masks.append(start_label_mask)
            end_label_masks.append(end_label_mask)
        else:
            start_label_mask = [1] * max_context_length
            end_label_mask = [1] * max_context_length
            start_label_masks.append(start_label_mask)
            end_label_masks.append(end_label_mask)
   ...
   #pdb.set_trace()
    ...
    #pdb.set_trace()
    # start_label_mask, _ = select_field_with_padding(processed_samples, "start_label_mask", pad_idx=0)
    ...
    start_label = torch.zeros(context_vecs.size(), dtype=context_vecs.dtype)
    end_label = torch.zeros(context_vecs.size(), dtype=context_vecs.dtype)
    start_label_mask = torch.tensor(start_label_masks, dtype=context_vecs.dtype)
    end_label_mask = torch.tensor(end_label_masks, dtype=context_vecs.dtype)


    for i, mention_idx_vec in enumerate(mention_idx_vecs):
        for j, pos in enumerate(mention_idx_vec):
            if mention_idx_mask[i][j]:
                start_pos, end_pos = pos
                start_label[i][start_pos] = 1
                end_label[i][end_pos] = 1
    ...
    tensor_data_tuple = (context_vecs, cand_vecs, label_idx, start_label, end_label, mention_idx_vecs, mention_idx_mask, label_idx_mask, start_label_mask, end_label_mask)
    # save data
...
def get_context_representation_multiple_mentions_left_right(
    ...
    if len(all_mentions[0]) == 0 and len(all_context_lefts[0]) == 0 and len(
            all_context_rights[0]) == 0:  # passed in empty string
