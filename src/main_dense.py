...
import json
import sys
from .index.faiss_indexer import DenseFlatIndexer, DenseHNSWFlatIndexer, DenseIVFFlatIndexer
...
from .candidate_ranking import utils as utils
import math
...
tokenizer = BertTokenizer.from_pretrained("bert-base-uncased", do_basic_tokenize=True, is_split_word=True)
...
def _decode(token_ids, input_sample_ids, start_id, end_id):
    start_sample_id = input_sample_ids[start_id]
    end_sample_id = input_sample_ids[end_id]
    restored_text = [''] * (end_sample_id - start_sample_id + 1)
    tokens = tokenizer.convert_ids_to_tokens(token_ids)
    old_id = start_sample_id
    position = 0
    for i, order in enumerate(input_sample_ids):
        if order >= start_sample_id and order <= end_sample_id:
            if order != old_id:
                position += 1
            restored_text[position] += tokens[i].replace('##', '')
            old_id = order
...
def _process_biencoder_dataloader(samples, tokenizer, biencoder_params, logger):
    ...
    if 'label_id' in samples[0]:
        ...
        for sample in samples:
            ...
            encoded_sample = [101]
            sample_id_list = [-1]
            for i, tokens in enumerate(sample['text'].split()):
                token_list = tokenizer.encode(tokens)[1:-1]
                sample_id_list = sample_id_list + [i] * len(token_list)
                encoded_sample = encoded_sample + token_list
            encoded_sample = encoded_sample + [102]
            sample_id_list += [-1]
            samples_text_order.append(sample_id_list)
            samples_text_tuple.append(encoded_sample)
        ...
        for i, sample in enumerate(samples):
            ...
            if k > 1000:
                break
            encoded_sample = samples_text_tuple[i]
            samples_text_tuple[i] = encoded_sample + [0 for _ in range(max_seq_len - len(encoded_sample))]
            samples_text_order[i] = samples_text_order[i] + [0 for _ in range(max_seq_len - len(encoded_sample))]

        tensor_data_tuple = [torch.tensor(samples_text_tuple), torch.tensor(samples_text_order)]
...
def get_predictions(
    ...
    for batch_num, batch_data in enumerate(dataloader):
        ...
        for b in range(len(batch_context)):
            ...
            input_sample_ids = batch_order[b][batch_context[b] != 0].tolist()
            ...
            input_sample_ids = input_sample_ids[1:-1]
            pred_triples = [(
            ...
            if 'label_id' in sample:
                ...
                entity_results.update({
                    "pred_tuples_string": [
                        [id2title[triple[0]], _decode(input_context, input_sample_ids, triple[1], triple[2])]
                        for triple in pred_triples
                    ],
                    "pred_triples": pred_triples,
                    "tokens": input_context,
    ...
    if args.interactive:
        while not stopping_condition:
            ...
            dataloader = _process_biencoder_dataloader(
                samples, tokenizer, biencoder_params, logger,
            )
            # dataloader = _process_biencoder_dataloader(
        ...
        # )
        dataloader = _process_biencoder_dataloader(
            samples, tokenizer, biencoder_params, None,
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="models/elq_large_case_params.txt",
