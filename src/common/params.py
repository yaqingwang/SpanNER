...
# Provide an argument parser and default command line options for using SpanNER.
import argparse
import importlib
import os
import sys
...
class SpanParser(argparse.ArgumentParser):
    """
    ...
    :param add_span_args:
        (default True) initializes the default arguments for SpanNER package.
    ...
    def __init__(
        self, add_span_args=True, add_model_args=False,
        description='SpanNER parser',
    ):
        super().__init__(
            ...
            add_help=add_span_args,
        )
        self.Span_home = os.path.dirname(
            os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
        )
        os.environ['span_HOME'] = self.Span_home
    ...
    def add_span_args(self, args=None):
        """
        Add common SpanNER args across all scripts.
        """
        ...
        parser.add_argument(
            "--use_attention",
            action="store_true",
            help="Whether to use attention to aggregate candidate embeddings.",
        )
        parser.add_argument(
            "--zero_shot",
            action="store_true",
            help="Whether to test in the zero-shot setting.",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--seed", type=int, default=42, help="random seed for initialization 52313"
        )
        parser.add_argument(
            "--zeshel",
            default=True,
    ...
    def add_model_args(self, args=None):
        ...
        parser.add_argument(
            "--entity_inference",
            action="store_true",
            help="Whether to test inference only.",
        )
        parser.add_argument(
            "--data_path",
            default="",
